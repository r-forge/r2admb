...
depending on your platorm: at the moment,
...
ADMB installation (I hope this Just Works once you have installed ADMB, but there's a chance that things will have to be tweaked).
...
\item run \code{do\_admb(fn,data,params)}, where \code{fn} is the base name (without extension) of your TPL file, \code{data} is a list of the input data, and \code{params} is a list of the starting parameter values; if you want R to generate the PARAMETERS and DATA section of your TPL file automatically, use \code{do\_admb(fn,data,params,checkparam="write",checkdata="write")}. Save the results as an R object.
\item use the standard R model accessor methods (\code{coef}, \code{summary}, \code{vcov}, \code{logLik}, \code{AIC}) to explore the results.
\end{enumerate}
...
See the documentation for the \code{coda} package.
...
we have to specify \code{re=TRUE} (to turn on random effects) and
we have to use the \code{re\_vectors}
...
d1 <- do_admb("toy1",
              ...
              re_vectors=list(u_herd=ncol(Zherd)),
